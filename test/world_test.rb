...
class WorldTest < Test::Unit::TestCase
  ...
  def test_world_knows_about_diagonal_neighbors
    cell = Cell.new
    world = new_world [ [Cell.new, cell, Cell.new], [Cell.new, Cell.new, Cell.new] ]
    assert_equal 5, world.neighbors_of(cell).count
  end

  def test_world_knows_about_live_neighbors
    cell = Cell.new
    assert_equal 1, new_world([ [cell], [live_cell] ]).live_neighbors_of(cell).count
  end

  def test_world_should_wrap_the_inner_horizontal_edge_when_counting_neighbors
    cell = Cell.new
    assert_equal 2, new_world([ [cell, Cell.new, Cell.new] ]).neighbors_of(cell).count
    assert_equal 5, new_world([ [cell, Cell.new, Cell.new], [Cell.new, Cell.new, Cell.new ]]).neighbors_of(cell).count
  end

  def test_world_should_wrap_the_outer_horizontal_edge_when_counting_neighbors
    cell = Cell.new
    assert_equal 2, new_world([ [Cell.new, Cell.new, cell] ]).neighbors_of(cell).count
  end

  def test_world_should_wrap_the_lower_vertical_edge_when_counting_neighbors
    cell = Cell.new
    assert_equal 2, new_world([ [cell],
                                [Cell.new],
  ...
  def test_world_should_wrap_the_upper_vertical_edge_when_counting_neighbors
    cell = Cell.new
    assert_equal 2, new_world([ [Cell.new],
                                [Cell.new],
